...
type LinkedInLoginDemoProps = {
        user: User | null;
}

export default function LinkedInLoginDemo({ user }: LinkedInLoginDemoProps) {
    ...
    const handleLinkedInSignIn = async () => {
        const { error } = await supabase.auth.signInWithOAuth({
            ...
            options: {
                redirectTo: `${window.location.origin}/linkedin-login`,
                skipBrowserRedirect: false, // Ensure the redirect happens in the same tab
            }
        });
    };
    ...
    return (
       <AuthDemoPage
         ...
         intro="Authenticate using LinkedIn—Supabase handles the OAuth flow and returns a ready-to-use session."
         steps={[
           "Click the button to sign in with LinkedIn.",
